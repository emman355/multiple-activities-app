  ...
  return (
    <div>
      <Dropzone
        ...
        src={value ? [value] : []}
        maxFiles={1}
        className="bg-accent hover:bg-muted-foreground"
      >
        <DropzoneContent>
          {filePreview && (
            <div className="h-[150px]">
              {!loaded && <Skeleton className="absolute inset-0 rounded-xs" />}
              <Image
                alt="Preview"
                src={filePreview}
                layout="fill"
      ...
      <Typography variant="small">Drag and drop or click to replace</Typography>
    </div>
  );
};
