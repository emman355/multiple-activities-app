...
export default async function PokemonReviews({
  name,
  details,
}: {
  name: string;
  ...
  const filteredReviews: PokemonReview[] = pokemonReviews.filter(
    (review: { pokemonName: string }) => review?.pokemonName?.toLowerCase() === name
  );
  ...
  return (
    <div className="flex flex-col justify-start gap-5">
      <div className="flex justify-between">
        ...
        {filteredReviews.length !== 0 && <AddReview pokemonName={name} details={details} />}
      </div>
      {filteredReviews.length ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 border border-border rounded-2xl bg-muted/30">
          <FaRegCommentDots className="text-accent mb-4" size={40} />
          ...
          <Typography variant="body2" className="text-muted-foreground mb-6 text-center max-w-sm">
            Be the first to share your thoughts and rate{' '}
