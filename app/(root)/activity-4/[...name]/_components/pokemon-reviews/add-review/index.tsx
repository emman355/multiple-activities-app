...
export default function AddReview({ pokemonName, details }: { pokemonName: string, details: PokemonDetails }) {
  ...
  return (
    <div>
      <Button className='bg-cyan-800 hover:bg-cyan-900' onClick={() => setopenAddReview(true)}>
        <FaPlusCircle />
        <Typography variant='small' className='font-semibold'>Add Pokemon Review</Typography>
      </Button>
      <CustomDialog
        open={openAddReview}
          ...
          <div className='flex-1 flex-col flex items-center gap-4'>
            <div className="h-70 relative w-full rounded-3xl bg-gray-800">
              <Image
                ...
                unoptimized
              />
              <Typography variant='subtitle' className='absolute bottom-1 left-[calc(50%-1.25rem)]'>{`#${formatPokemonId(details.id)}`}</Typography>
            </div>
          </div>
